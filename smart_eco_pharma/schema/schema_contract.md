...
| `test_method` | TEXT (enum) | CHECK (`hplc`, `gc`, `uv_vis`, `ftir`, `titration`, `mass_spectrometry`, `other`) | YES | `NULL` | **Eman** | The analytical method used for testing. Must be one of the controlled values listed — no free-text. Pick `other` if the method isn't in the list yet, then fill in `test_method_other_description` below to explain what it is. **Controlled values:** `hplc` (High-Performance Liquid Chromatography), `gc` (Gas Chromatography), `uv_vis` (UV-Visible Spectroscopy), `ftir` (Fourier-Transform Infrared Spectroscopy), `titration`, `mass_spectrometry`, `other`. |
| `test_method_other_description` | TEXT | — | YES | `NULL` | **Eman** | Only fill this in when `test_method` is set to `other`. Describe the actual analytical method here (e.g., "Karl Fischer Titration", "X-Ray Diffraction"). Once a method from this column is used enough times, the architecture team can promote it to a permanent enum value in a future migration. Leave blank if `test_method` is not `other`. |
